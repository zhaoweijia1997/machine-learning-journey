...
学完前3课，你可能会有一些更深层的问题。这里整理了一些常见的思考。
...
这就是为什么神经网络被称为**"通用函数逼近器"**——理论上可以拟合任意复杂的函数。
...
| 层数 | 优点 | 缺点 |
|------|------|------|
| 少 | 简单、快、不容易过拟合 | 表达能力有限 |
| 多 | 能学习更复杂的模式 | 需要更多数据、更难训练 |
...
它从海量数据中找到重复出现的模式，然后应用到新数据上。如果某件事没有规律，或者规律从未在训练数据中出现过，AI 就无能为力。
